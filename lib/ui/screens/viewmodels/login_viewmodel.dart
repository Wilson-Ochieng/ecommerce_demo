  ...
  Future<bool> login({required String email, required String password}) async {
    ...
    try {
      ...
      if (token != null && token.isNotEmpty) {
        try {
          await _authRepository.updateFcmToken(uid: _user!.uid, token: token);

          debugPrint('FCM token saved for user ${_user!.uid}');
        } catch (e) {
      ...
      _notificationService.listenForTokenRefresh((newToken) async {
        try {
          ...
          debugPrint('FCM token refreshed for user ${_user!.uid}');
        } catch (e) {
